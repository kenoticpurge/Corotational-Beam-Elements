...
% The structure and formulations are the exact same as the cantilever case.
% The only difference is that the "Gravity Vector" is added to Fg

% Matrix reset to prevent unesscesarry addtion
Kk = zeros(params.n);
M = zeros(params.n);
Ck = zeros(params.n);
Kg = zeros(params.n);
Fg = zeros(params.n,1); % Internal Force Vector
FK = zeros(params.n,1); % Inertia Force Vector
...
for ie = 1:params.ne
    ...
    % configuration
    [T,B,r,z,Ln,Lo,qbar,cos] = EL_getKinematics(q0e,qe);
    
    tbar = qbar(2:3);
    % Obtain the Element Mass matrix in the Corotational Frame
    ML = EL_massmatrix(params,Lo,tbar);
    
    % Transform Element Mass Matrix into the Global Frame
    Me = T' * ML * T;

    % Element Inertia Force Vector
    fKe = EL_InertiaForceVector(params,ML,T,B,z,Ln,Lo,Me,qde,qdde);

    % Obtain the Global Tangent Stiffness Matrix and the Local Elastic
    % Force Vector for the element
    [fle, Kge] = EL_globstiffmatrix(params,B,r,z,Ln,Lo,qbar);
    % Element Global Elastic Force Vector
    fge = B'*fle;
    % Element gravity force vector
    fgp = EL_gravityvector(params,Lo,Ln,cos);
    ...
    % Assemble matrices of the overall structure
    ii = (ie-1)*3 + 1;
    ...
    % Inertia Centrifugal Stiffness Matrix Assembly
    ...
    Fg(istart:iend) = Fg(istart:iend) + (fge + fgp);
